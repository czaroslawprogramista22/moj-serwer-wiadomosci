...
db.serialize(() => {
    ...
    // Indeks przyspieszający wyszukiwanie przy 100 oknach
...
// Zapytanie z licznikiem unikalnych account_id dla alertów multikont
...
app.post('/log', (req, res) => {
    ...
    const data = new Date().toISOString(); // Zapis ISO dla poprawnego sortowania
...
app.post('/api/usun', sprawdzLogowanie, (req, res) => {
    db.run("DELETE FROM wiadomosci WHERE id = ?", req.body.id, (err) => {
        if (err) return res.status(500).send("Błąd");
        res.json({ status: "ok" });
    });
});
